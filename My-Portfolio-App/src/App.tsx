  ...
  2026: [
    {
      ...
      desc: "Spatial analytics dashboard for real-time IoT sensor data across Metro Manila.",
      tags: ["React", "Node.js", "WebSockets"],
    },
    {
      id: 2,
      title: "PayChain",
      desc: "Blockchain-backed payroll system for SMEs in Southeast Asia.",
      tags: ["Solidity", "Next.js", "PostgreSQL"],
    },
  ],
  2025: [
    {
      id: 3,
      title: "Lumen Mobile",
      desc: "Circadian-rhythm mindfulness app with adaptive notifications.",
      tags: ["React Native", "Supabase"],
    },
    {
      id: 4,
      title: "Atlas Platform",
      desc: "Interactive climate storytelling platform.",
  ...
  2024: [
    {
      id: 5,
      title: "ShopOS",
      desc: "E-commerce OS with POS, inventory, and analytics unified.",
      tags: ["TypeScript", "Prisma", "Stripe"],
    },
    {
      id: 6,
      title: "DevPulse",
      desc: "GitHub-integrated dev activity tracker with Slack digest reports.",
      tags: ["Go", "GitHub API"],
    },
  ],
};
...
const EXPERIENCE = [
  {
    ...
    period: "2024 — Present",
    desc: "Building production-ready web and mobile apps. Specializing in React, Node.js, and cloud systems.",
  },
  {
    role: "Junior Software Engineer",
    company: "TechBridge PH",
    period: "2023 — 2024",
    desc: "Developed internal dashboards and APIs. Migrated legacy codebases to Next.js + PostgreSQL.",
  },
];
...
type Skill = { name: string; src?: string; custom?: "sql" | "nosql" };
const SKILLS: Skill[] = [
  ...
  {
    name: "Express.js",
    src: `${DEVICON_BASE}/express/express-original-wordmark.svg`,
  },
  { name: "SQL", custom: "sql" },
  ...
  { name: "GitHub", src: `${DEVICON_BASE}/github/github-original.svg` },
    ...
    <div className="fixed top-0 left-0 right-0 z-[70] h-[3px] bg-transparent pointer-events-none">
      <div
        ...
        style={{
          ...
          animation: "siriFlow 5s linear infinite",
          boxShadow:
            "0 0 14px 1px rgba(191,90,242,0.55), 0 0 6px 1px rgba(10,132,255,0.55)",
...
function GithubDotContribution({ dark, T }: { dark: boolean; T: Theme }) {
  ...
  const [weeksData, setWeeksData] = useState<
    { w: number; days: { level: number; count: number; date: string }[] }[]
  >([]);
    ...
    async function fetchGitHubData() {
      try {
        ...
        for (let i = 0; i < 364; i++) {
          ...
          let level = 0;
          let count = 0;
          if (rand > 0.52 && rand <= 0.72) {
            level = 1;
            count = Math.floor(rand * 3) + 1;
          } else if (rand > 0.72 && rand <= 0.88) {
            level = 2;
            count = Math.floor(rand * 6) + 3;
          } else if (rand > 0.88 && rand <= 0.96) {
            level = 3;
            count = Math.floor(rand * 10) + 7;
          } else if (rand > 0.96) {
            level = 4;
            count = Math.floor(rand * 16) + 12;
          }
          total += count;
    ...
    <div className="w-full flex flex-col gap-3">
      <div className="flex items-center justify-between text-xs font-mono px-1">
        ...
        <a
          ...
          style={{ color: T.muted }}
        >
          <GithubMark size={12} color={T.muted} />@{GITHUB_USERNAME} ↗
        </a>
      </div>
      ...
      >
              ...
              >
                {week.days.map((day, idx) => (
                  <div
                    key={idx}
                    onMouseEnter={() =>
                      setHoveredDot({ date: day.date, count: day.count })
                    }
        ...
        <div
          className="mt-3 pt-2.5 flex items-center justify-between border-t text-[10px] font-mono"
          style={{ borderColor: T.glassBorder, color: T.muted }}
        >
          <div>
            {hoveredDot ? (
              <span className="text-white font-medium">
                {hoveredDot.count} commits{" "}
                {hoveredDot.date ? `on ${hoveredDot.date}` : ""}
              </span>
            ) : (
...
function ProjectCard({
  ...
  p: { id: number; title: string; desc: string; tags: string[] };
  T: Theme;
}) {
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <p className="text-xs font-light mb-2" style={{ color: T.muted }}>
          {p.desc}
        </p>
        <div className="flex gap-1.5">
          {p.tags.map((t) => (
            <span
              key={t}
              className="px-2 py-0.5 text-[9px] font-mono rounded bg-white/5 text-gray-400"
            >
              {t}
            </span>
          ))}
...
function SkillBadge({ skill, T }: { skill: Skill; T: Theme }) {
    ...
    >
      <div
        ...
        style={{
          ...
          border: `1px solid ${T.glassBorder}`,
          boxShadow: hover
            ? "0 8px 24px rgba(191,90,242,0.28), 0 0 0 1px rgba(191,90,242,0.25)"
      ...
      >
        {skill.custom ? (
          ...
          <img
            ...
            height={24}
            style={{
              filter: hover ? "none" : "grayscale(1) opacity(0.75)",
    ...
    <div className="space-y-6">
      ...
      {YEARS.map((y) => (
        <section key={y} style={bubble(T)} className="p-6">
          <p className="text-[10px] font-mono mb-4" style={{ color: T.muted }}>
            {y}
          </p>
    ...
    <div className="space-y-6">
      ...
      <section style={bubble(T)} className="p-6 sm:p-8 space-y-5">
        <p className="text-[10px] font-mono" style={{ color: T.muted }}>
         2020 - Back when I was in Grade 7
        </p>
        <p
          className="text-sm font-light leading-relaxed"
          style={{ color: T.text }}
        >
        Instead of getting frustrated over pop-up errors like "Error 0xc000007b" or missing .dll files, I became fascinated by why they happened in the first place. I spent hours searching YouTube for fix tutorials, tweaking system files, and troubleshooting compatibility issues until the game finally launched. That was the moment I realized I enjoyed the problem-solving behind the scenes far more than playing the actual game.
        </p>
         <p className="text-[10px] font-mono" style={{ color: T.muted }}>
         2020 - Back when I was in Grade 7
        </p>
        <p
          ...
          style={{ color: T.text }}
        >
          School — through Mariano Marcos State University and later the
          Polytechnic University of the Philippines — shaped how I think about
          building things: slow down, get the fundamentals right, then move
          fast. That habit followed me into Manila, where I traded textbooks for
          production bugs and learned more in six months of shipping than in
          years of tutorials.
        </p>
        <p
          className="text-sm font-light leading-relaxed"
          style={{ color: T.text }}
        >
          Today I build full-stack products — React and Next.js on the front,
          Node and databases underneath — and I still get the same rush from a
          clean deploy that I did from my first working "Hello World."
        </p>
    ...
    >
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .floating-bot, .page-enter, [style*="siriFlow"] { animation: none !important; }
        }
      `}</style>
      ...
      <main
        key={page}
        className="page-enter relative max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-8"
      >
          ...
          <>
            ...
            <section className="relative min-h-[420px] flex items-center justify-center">
              <div
                ...
                onClick={() => playIosClickSound()}
                style={{
                  transform: `translate3d(${robotX}px, ${robotY}px, 0)`,
                }}
              >
              ...
              <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <p
                  className="mt-3 text-xs sm:text-sm max-w-md leading-relaxed"
                  style={{ color: T.muted }}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <section
                ...
                style={{
                  backgroundColor: dark ? "#0a0a0c" : "#ffffff",
                  borderColor: T.glassBorder,
                }}
              >
                <p className="text-[10px] font-mono mb-4 text-gray-500">
                  00 — PHILOSOPHY
                </p>
              ...
              >
                ...
                <p
                  ...
                  style={{ color: T.text }}
                >
                  Junior Full Stack Developer based in Metro Manila. Focused on
            ...
            <section style={bubble(T)} className="p-6">
              ...
              {EXPERIENCE.map((e, i) => (
                <div
                  key={i}
                  onClick={() => playIosClickSound()}
                  className="py-3 border-b last:border-none cursor-pointer click-active"
                  style={{ borderColor: T.glassBorder }}
                >
                  ...
                  <p className="text-xs font-light" style={{ color: T.muted }}>
                    {e.desc}
                  </p>
                </div>
              ))}
            ...
            <section style={bubble(T)} className="p-6">
              ...
              <div className="flex flex-wrap gap-4">
                {SKILLS.map((s) => (
                  <SkillBadge key={s.name} skill={s} T={T} />
                ))}
              </div>
            </section>
                  ...
                  <div className="flex gap-1">
                    {YEARS.map((y) => (
                      <button
                        ...
                        style={{
                          backgroundColor: year === y ? T.text : "transparent",
                          color: year === y ? T.bg : T.muted,
                        }}
                      >
                        {y}
                      </button>
            ...
            <section
              id="contact"
              onClick={() => playIosClickSound()}
              className="p-8 rounded-3xl border text-center cursor-pointer click-active"
              style={bubble(T)}
            >
              <p
              ...
              <p className="text-xs mt-2 mb-5" style={{ color: T.muted }}>
                Open for software roles and projects.
              </p>
              <a
                href="mailto:aaron@example.com"
                className="inline-block px-6 py-2.5 rounded-full text-xs font-medium text-white"
            ...
            <footer
              className="text-center py-6 text-[10px] font-mono"
              style={{ color: T.muted }}
            >
...
function PremiumRobot({
  ...
  return (
    ...
    >
      ...
      <path
        d="M32 53C32 53 36 58 42 58C48 58 52 53 52 53"
        stroke={stroke}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
...
function PremiumDrone({
  ...
  return (
    ...
    >
      ...
      <path
        d="M27 30L34 36M57 30L50 36M27 54L34 48M57 54L50 48"
        stroke={stroke}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
